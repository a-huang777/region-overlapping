...
from global_guidance import GlobalUncoveredHeatmap
...
class Config:
    ...
    guidance_top_k = 1  # 每步显示的引导点数量（仅展示，不并入状态）
    guidance_min_grid_dist = 6  # 引导点最小网格间距
    guidance_edge_penalty_weight = 2  # 边界惩罚权重，越大越不容易在边缘产生引导点
    guidance_reach_dist = 5.0  # 与当前引导点距离小于该值(米)时切换下一个引导点
...
class MultiUAVCoverageEnv(gym.Env):
    ...
    def __init__(self, cfg=Config()):
        ...
        self.steps = 0
        self.episode_idx = 0
        self.guidance = GlobalUncoveredHeatmap(
    ...
    def step(self, action):
        ...
        # 4. 引导点切换逻辑：
        # 仅当接近当前引导点（< guidance_reach_dist）或当前引导点无效时才刷新/切换。
        if pre_guidance_world is None:
            self._refresh_guidance_points()
        else:
            dist_to_current = self._distance_to_guidance(self.agents_pos[0], pre_guidance_world)
            if dist_to_current < self.cfg.guidance_reach_dist:
                if self.current_guidance_idx + 1 < len(self.current_guidance_points):
                    self.current_guidance_idx += 1
                else:
                    self._refresh_guidance_points()
    ...
    def _refresh_guidance_points(self):
        self.guidance.update_from_global_map(self.global_map)
        self.current_guidance_points = self.guidance.get_uncovered_region_guidance_points(
            k=self.cfg.guidance_top_k,
            min_grid_dist=self.cfg.guidance_min_grid_dist,
            # edge_penalty_weight=self.cfg.guidance_edge_penalty_weight,
        )
        self.current_guidance_idx = 0
