"""
Run the trained policy and render a simple 3D simulation.
...
class SingleUAVSB3Wrapper(gym.Wrapper):
    """Adapt MultiUAVCoverageEnv (n=1) to SB3 single-agent interface."""
...
def build_env(max_steps: int) -> tuple[MultiUAVCoverageEnv, SingleUAVSB3Wrapper]:
    env_cfg = Config()
...
def run_policy_rollout(
    base_env: MultiUAVCoverageEnv,
    wrapped_env: SingleUAVSB3Wrapper,
    model: PPO,
    seed: int,
) -> RolloutResult:
    obs, reset_info = wrapped_env.reset(seed=seed)
    terminated = False
    truncated = False
    steps = 0
    total_reward = 0.0
    ...
    coverage_rate = float(info.get("agent_0", {}).get("coverage_rate", 0.0))
    if coverage_rate >= base_env.cfg.coverage_goal:
...
def make_ground_mesh(base_env: MultiUAVCoverageEnv) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    edges = np.linspace(0.0, base_env.cfg.env_size, base_env.cfg.grid_size + 1)
...
def draw_scene(
    ...
    x_mesh, y_mesh, z_mesh = make_ground_mesh(base_env)
    ax.plot_surface(
        x_mesh,
        y_mesh,
        z_mesh,
    ...
    draw_circle3d(ax, pos, 0.12, base_env.cfg.det_radius, DETECTION_COLOR, 0.75, 2.0, SCAN_ZORDER)
    ...
    ax.scatter([base_env.start_pos[0]], [base_env.start_pos[1]], [0.2], c=START_COLOR, marker="o", s=52, zorder=GUIDANCE_ZORDER)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render a 3D simulation for the coverage policy.")
    parser.add_argument("--model", default="model/ppo_model_last.zip", help="Path to the trained PPO model.")
    ...
    parser.add_argument("--max-steps", type=int, default=500, help="Maximum environment steps per episode.")
    ...
    parser.add_argument("--frame-stride", type=int, default=1, help="Render every Nth recorded frame.")
...
def main() -> None:
    ...
    base_env, wrapped_env = build_env(max_steps=args.max_steps)
    print(f"Loading model: {model_path}")
    model = PPO.load(str(model_path), env=wrapped_env, device="cpu")

    for episode_idx in range(1, args.episodes + 1):
        seed = args.seed + episode_idx
        print(f"Running episode {episode_idx} with seed {seed}...")
        result = run_policy_rollout(base_env, wrapped_env, model, seed=seed)
        print(
            f"Episode {episode_idx}: end={result.end_reason}, "
            f"coverage={result.coverage_rate * 100:.2f}%, "
            f"steps={result.high_steps}, frames={len(result.frames)}, "
